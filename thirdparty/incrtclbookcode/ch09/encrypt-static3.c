 ...
 * Encrypt_Init for the example on modifying data members from C.
 */
  ...
  Tcl_Obj *CONST objv[];  /* array of argument strings     */
{
  ...
  char* name = "_maxLen"; /* Name of class data member              */
  char* varRef;           /* Local reference to data member         */
  int maxLen = 0;         /* Integer equivalent of data member      */
  ...
  /*
   * Get the data member from the object and convert to an int.
   */
  varRef = Tcl_GetVar(interp,name,TCL_NAMESPACE_ONLY);
  if (varRef)
  {
    Tcl_GetInt(interp,varRef,&maxLen);
    if (!maxLen)
    {
      return TCL_ERROR;
    }
  ...
  {
    Tcl_AppendResult(interp, "invalid variable access: _maxLen ",
      "does not exist or is not initialized", (char *)NULL);
    return TCL_ERROR;
  }
  ...
  Tcl_SetVar(interp, "_ciphertext", ciphertext, TCL_NAMESPACE_ONLY);
  Tcl_Free(ciphertext);
  return TCL_OK;
}
